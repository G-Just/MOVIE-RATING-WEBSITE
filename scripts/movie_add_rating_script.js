...
if (document.getElementById('add-rating-rate') !== null){
    ...
    const searchMoviesAdvanced = async (title, type, year) => {
        const response = await fetch(`${API_URL}&t=${title}&plot=full&type=${type}&y=${year}`)
        const data = await response.json();

        document.getElementById('genre').innerText = data['Genre']
        document.querySelector("[name=genre]").setAttribute('value', data['Genre'])

        document.getElementById('imdb').innerText = data['imdbRating']+" ("+data['imdbVotes']+" votes)"
        document.querySelector("[name=imdb]").setAttribute('value', data['imdbRating'])
        ...
        document.getElementById('type').innerText = capitalizeFirstLetter(data['Type'])
        document.querySelector("[name=type]").setAttribute('value', data['Type'])

        document.querySelector("[name=poster]").setAttribute('value', data['Poster'])
    ...
    try {
        document.getElementById('remove-rating-button').addEventListener('click', e => {
