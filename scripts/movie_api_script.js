    ...
    const searchMovies = async (title) => {
        ...
        if (data['Response'] === 'True'){
            ...
            for(i=0; i < data['Search'].length; i++){
                ...
                else {movie.innerHTML = `<img src='media/movie-projector-smoke.jpg' style='object-fit: cover;'></img>`}
                    movie.innerHTML += `<h4>${data["Search"][i]["Title"]}</h4>
                                        <p>${capitalizeFirstLetter(data["Search"][i]["Type"])}<span>${data["Search"][i]["Year"]}</span></p>`
